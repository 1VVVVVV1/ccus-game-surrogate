...
def stage_outcome(mode, scenario, carbon_price, state, action):
    ...
    ou = math.fsum((u["fixed_opex"] * x_u, u["utilization_cost_per_ton"] * q_u, u["storage_cost_per_ton"] * q_s))
    ...
    direct = math.fsum((rc, oil, subsidy, -oc, -ou, -ct, -ic, -iu))
    ...
    if mode == "TRANSFER":
        ...
        pc = math.fsum((rc, payment_u, -transport_payment, -payment_s, -oc, -ic))
        pu = math.fsum((oil, -payment_u, payment_s, subsidy, -ou, -iu))
        pt = transport_payment - ct
        system = math.fsum((pc, pu, pt))
        error = abs(system - direct)
    elif mode == "JOINT_VENTURE":
        pool = math.fsum((rc, payment_u, subsidy, -oc, -ct, -u["fixed_opex"] * x_u, -u["storage_cost_per_ton"] * q_s, -ic, -iu))
        application = math.fsum((oil, -payment_u, -u["utilization_cost_per_ton"] * q_u))
        shares = CONFIG["joint_venture"]
        pc = shares["theta_C"] * pool
        pt = shares["theta_T"] * pool
        pu = application + shares["theta_U"] * pool
        system = math.fsum((pool, application))
        error = max(abs(system - math.fsum((pc, pu, pt))), abs(system - direct))
    ...
    if not static_accounting_passes(error):
